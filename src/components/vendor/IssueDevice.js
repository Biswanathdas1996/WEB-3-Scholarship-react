...
export default function IssueDevice() {
    ...
    const findStudentByRoll=()=>{
      let newStudenList=studentList.filter(student=>student.roll_no==rollNo);
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
                ...
                <Divider sx={{ my: 1 }} />
                <Grid item xs={8}>
                  <TextField id="outlined-basic" name="roll" label="Enter Roll no" variant="outlined" onChange={e=>setRollNo(e.target.value)} />
                  <Button variant="outlined" className='ml-2' onClick={findStudentByRoll}>Search</Button>
                </Grid>
                  ...
                  <TableBody>
                    {studentList.map((row) => (
                      <TableRow key={row.sl_no}>
                        ...
                        <TableCell align="right"><Link to={`/assign-device/${row.id}`}><Button variant="outlined">Issue</Button></Link></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
